...
export const DiagnosisWidget: React.FC = () => {
  ...
  const calculateResult = (ans: Record<number, string>) => {
    ...
    if (q1 === 'just_passed') {
      setResult({
        title: '【緊急診断】まず通帳・重要書類を確保し、四十九日後にプロへご相談がベスト！',
        conclusion: '焦って家具を捨てず、まずは書類整理と公共料金の手続きだけ行いましょう。遠方または時間が取れない場合は自力で抱え込まず、専門の無料相談窓口をご活用いただくのが最善です。',
        steps: [
          '1. 通帳・権利証・遺言書の有無を優先して確認する',
          '2. 電気・水道は契約を残し、固定電話や新聞を解約する',
          '3. 不用品処分・遺品整理は無料相談窓口へ一括相談する',
        ],
        recommendedArticleSlug: 'jikka-seiri-first-step',
        recommendedArticleTitle: '親が亡くなった直後の実家整理「何から始める？」後回しにしない順番と全手順',
        ctaText: '無料で査定・見積もり相談してみる',
      });
    } else if (q1 === 'empty_home') {
      setResult({
        title: '【空き家放置対策】特定空き家による税金増額を防ぐ「現状無料査定・買取相談」が最適！',
        conclusion: '放置は固定資産税が最大6倍になるリスクがあります。現状のままでの売却・買取査定を取り寄せることで、将来の負担を大幅に削減できます。',
        steps: [
          '1. 実家の維持費用（毎年の税金・草刈代）を把握する',
          '2. 空き家・中古戸建ての無料買取査定・見積もりを取得する',
          '3. 特例控除（3,000万円控除）が使える3年以内にご相談する',
        ],
        recommendedArticleSlug: 'akiya-houchi-tax-risk',
        recommendedArticleTitle: '誰も住まない実家を放置すると固定資産税が6倍に？「特定空き家」のリスクと回避策',
        ctaText: '無料で査定・買取相談してみる',
      });
    } else if (q1 === 'too_much_stuff' || q2 === 'far') {
      setResult({
        title: '【遺品整理・家財処分】遠方・荷物多数ならプロの買取・一括整理が最適！',
        conclusion: '自力で数ヶ月通うよりも、専門の遺品整理・出張買取を活用する方が交通費と精神的負担を激減できます。買取可能な品があれば作業費用との相殺も可能です。',
        steps: [
          '1. 家族で「絶対に残したい形見」だけを先にピックアップ',
          '2. 不用品処分・着物や骨董等の高価買取査定を同時に依頼',
          '3. 1日の立ち会いのみで実家丸ごとスッキリ解決させる',
        ],
      ...
      setResult({
        title: '【売却・解体総合】相続登記を済ませつつ、現状買い取り・相見積もりをご比較！',
        conclusion: '親の名義のままでは契約できません。司法書士への相談と並行して、専門業者への現状査定・売却査定を取り寄せることが解決への一番近道です。',
        steps: [
          '1. 相続登記（名義変更）の手続きを進める',
          '2. 空き家・借地権・中古戸建ての現状買取価格を確認する',
          '3. 複数プランを比較して最適な方法を選択する',
        ],
    ...
    <div className="bg-stone-50 border-2 border-emerald-800/30 rounded-3xl p-5 md:p-8 shadow-md my-6">
      ...
      {result ? (
        <div className="bg-white rounded-2xl p-6 border-2 border-emerald-700 shadow-sm animate-fadeIn">
          ...
          <UnifiedCTA title="診断結果に合わせて無料で相談・査定してみる" buttonText={result.ctaText} />
